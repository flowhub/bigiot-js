...
class BigIotClient {
  constructor(market, corsproxy, options = {}) {
    this.market = market;
  ...
  authenticate() {
    ...
    return fetch(this.normalizeUrl(url.format(parsed)), { agent: this.options.httpAgent })
      .then((response) => {
  ...
  getClient(token) {
    ...
    const httpLink = createHttpLink({
      ...
      fetchOptions: { agent: this.options.httpAgent },
    });
