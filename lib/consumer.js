...
const allOfferings = gql`
  query allOfferings($active:Boolean!) {
    allOfferings(onlyActive: $active) {
      id name
...
const allOfferingsByCategory = gql`
  query allOfferings($category:String!, $active:Boolean!) {
    allOfferings(offeringCategoryUri: $category, onlyActive: $active) {
      id name
      rdfAnnotation { uri label }
      spatialExtent { city }
      inputs { name rdfAnnotation { uri } }
...
class BigIotConsumer extends Client {
  ...
  discover(category, active = true) {
    const query = category ? allOfferingsByCategory : allOfferings;
    return this.client.query({
      ...
      .then((result) => result.data.allOfferings);
  }
}
